...
def get_chapters(class_name: str, subject: str) -> List[Dict]:
    ...
    try:
        ...
        cache_key = f"{clean_class}_{subject.lower()}"
...
def get_books(class_name: Optional[str] = None, subject: Optional[str] = None) -> List[Dict]:
    ...
    """
    books = []
    clean_class = "".join(c for c in str(class_name) if c.isdigit()) if class_name else None
    ...
    try:
        ...
        for key, value in cache.items():
            ...
            cached_clean_class = "".join(c for c in str(cached_class_name) if c.isdigit())

            if clean_class and clean_class != cached_clean_class:
                continue
...
def save_book_details(class_name: str, subject: str, book_uuid: str, filename: str, chapters: List[Dict]):
    ...
    cache[cache_key]["class_name"] = clean_class
